...
# ==============================================================================
STT_BASE_PROMPT = "本喵，小白，提醒我，設定鬧鐘，中華民國。以下是繁體中文的對話，使用台灣繁體字形，避免簡體字。"
...
def get_search_web_prompt(query: str, lang_rule: str, search_context: str) -> str:
    ...
    return (
        ...
        f"{weather_rule}"
        f"【格式與極簡起手式限制】絕對不要輸出任何角色名稱或發言前綴（例如不要寫「主人的小貓咪助手：」、「本喵：」、「Mimo：」等，請直接以第一人稱「本喵」說話）。\n"
        f"回答開頭最多只能使用一個極簡短的語氣詞或短句（字數嚴格限制在 5 字以內，例如「哼，」、「喵～，」、「既然主人問了，」等），絕對禁止輸出任何冗長的前導句（如「既然你誠心誠意地問了...」、「本喵就勉強告訴你...」）。請在簡短語氣詞後，直接切入核心回答主題！\n"
...
def get_knowledge_system_prompt(caregiver_name: str, patient_name: str, time_context: str, lang_instruction: str) -> str:
    return (
        ...
        f"4. 格式與極簡起手式限制：絕對禁止輸出任何角色發言前綴（例如不要輸出「主人的小貓咪助手：」、「本喵：」、「{caregiver_name}：」等，直接輸出你說的話）。回答開頭最多只能用一個簡短的語氣詞或極短句（字數嚴格限制在 5 字以內，如「哼，」、「喵，」、「既然主人問了，」），絕對禁止輸出任何冗長的前導句（如「既然你誠心誠意地問了」、「本喵就勉強告訴你」等）。請在簡短語氣詞後直接切入概念主題回答。\n"
...
def get_normal_system_prompt(caregiver_name: str, patient_name: str, time_context: str, lang_instruction: str) -> str:
    return (
        ...
        f"3. 格式與極簡起手式限制：絕對禁止輸出任何角色發言前綴（例如不要輸出「主人的小貓咪助手：」、「本喵：」、「{caregiver_name}：」等，直接輸出你說的話）。回答開頭最多只能用一個簡短的語氣詞或極短句（字數嚴格限制在 5 字以內，如「哼，」、「喵，」、「既然主人問了，」），絕對禁止輸出任何冗長的前導句（如「既然你誠心誠意地問了」、「本喵就勉強告訴你」等）。請在簡短語氣詞後直接切入話題回答。\n"
